...
public class ScriptApi: ApiBase {
	...
	public ScriptApi(ScriptContainer source) : base(source) {
        Storage = new(source.Engine);
        StoragePath = Path.ChangeExtension(Path.Combine(Service.Interface.GetPluginConfigDirectory(), Owner.InternalName), "json");
	}
	...
	protected internal override void Init() {
        Owner.Engine.Options.DebugPrint = Debug.PrintString;
        Owner.Engine.Options.DebugInput = Debug.Input;
        Log("Loading script storage from disk", LogTag.ScriptLoader);
        ReloadStorage();
	}
	...
	public bool DeleteStorage([LuaDoc("If this is false, this method will always return true")] bool deleteDiskFile = true) {
        ...
        Log($"Deleting {StoragePath}", LogTag.ScriptStorage);
		try {
			...
			Service.Plugin.Error($"Failed to delete disk storage for {Owner.PrettyName}", err);
			return false;
		}
	}
	...
	public bool SaveStorage() {
        ...
        Owner.CleanTable(Storage);
        Log($"Writing to {StoragePath}", LogTag.ScriptStorage);
		try {
			File.WriteAllText(StoragePath, Storage.TableToJson());
			return true;
		}
		catch (Exception err) {
			Service.Plugin.Error($"Failed to save storage for {Owner.PrettyName}", err);
			return false;
		}
	}
	...
	public bool? ReloadStorage() {
        ...
        Log($"Loading from {StoragePath}", LogTag.ScriptStorage);
		try {
			string json = File.ReadAllText(StoragePath);
			Table loaded = JsonTableConverter.JsonToTable(json, Owner.Engine);
			if (loaded is null)
				return null;
            Owner.CleanTable(loaded);
            Storage = loaded;
			return true;
		}
		catch (FileNotFoundException) {
            Log("No disk storage found", LogTag.ScriptStorage);
			return false;
		}
		catch (SyntaxErrorException err) {
			Service.Plugin.Error($"Invalid JSON disk storage for {Owner.PrettyName}", err);
			return null;
		}
		catch (Exception err) {
			Service.Plugin.Error($"Failed to load storage for {Owner.PrettyName}", err);
			return null;
		}
	}
	...
	public void SetStorage([LuaDoc("The table to COPY as the new persistent storage")] Table replacement) {
        ...
        Log("Replacing script storage", LogTag.ScriptStorage);
		Table store = new(Owner.Engine);
        Owner.CleanTable(replacement);
	...
	public string Name => Owner.InternalName;
	...
	public string Title => Owner.PrettyName;
	...
	public string CallSelfCommand => Service.Configuration.RegisterDirectCommands ? $"/{Service.Configuration.DirectInvocationCommandPrefix}{Name}" : PluginCommand + " call " + Name;
	...
	public int QueueSize => Owner.ActionQueue.Count;
	...
	public void ClearQueue() => Owner.ActionQueue.Clear();
	...
	public void QueueDelay(uint milliseconds) => Owner.ActionQueue.Add(new PauseAction(milliseconds));
	...
	public void QueueAction(Closure callback, [AsLuaType(LuaType.Any), LuaDoc("arguments to be passed to the queued function call"), Optional] params DynValue[] arguments)
		=> Owner.ActionQueue.Add(new CallbackAction(DynValue.NewClosure(callback), arguments));
	...
	public void QueueAction(CallbackFunction callback, params DynValue[] arguments)
		=> Owner.ActionQueue.Add(new CallbackAction(DynValue.NewCallback(callback), arguments));
	...
	public Table? ParseJson(string jsonObject) {
        Log(jsonObject, LogTag.JsonParse);
		try {
			Table table = JsonTableConverter.JsonToTable(jsonObject, Owner.Engine);
            Owner.CleanTable(table);
			return table;
		}
		catch (SyntaxErrorException e) {
            Log(e.ToString(), LogTag.JsonParse);
			return null;
		}
	}
	...
	public string SerialiseJson(Table content) {
        Owner.CleanTable(content);
		string json = content.TableToJson();
        Log(json, LogTag.JsonDump);
		return json;
	}

	[SkipDoc("It's just a spelling variant of the above")]
	public string SerializeJson(Table content) => SerialiseJson(content); // american spelling
	...
	public bool HasPlugin(string pluginName) {
        DeprecationWarning("Game.Dalamud.HasPlugin(string)");
		return Owner.GameApi.Dalamud.HasPlugin(pluginName);
	}
	...
	private void showNotification(string content, NotificationType type, double durationModifier = 1) {
        ...
        Log($"Displaying notification (type {type}) of {content.Length:N} chars for {duration:N}ms ({initialDuration:D}ms x {durationModifier:F2})", LogTag.DebugMessage);
	...
	public void NotifyDebug(string content) {
		if (Debug.Enabled)
            showNotification(content, NotificationType.None);
	}
	[LuaDoc("Displays a Dalamud popup informational notification in the lower right corner.")]
	public void NotifyInfo(string content) => showNotification(content, NotificationType.Info);
	[LuaDoc("Displays a Dalamud popup success notification in the lower right corner.")]
	public void NotifySuccess(string content) => showNotification(content, NotificationType.Success);
	[LuaDoc("Displays a Dalamud popup warning notification in the lower right corner.")]
	public void NotifyWarning(string content) => showNotification(content, NotificationType.Warning, 1.1);
	[LuaDoc("Displays a Dalamud popup error notification in the lower right corner.")]
	public void NotifyError(string content) => showNotification(content, NotificationType.Error, 1.2);
	...
	public override string ToString() => $"Script[{Owner.PrettyName}]";
	...
	public void RegisterCallbackFunction(DynValue self, [AsLuaType(LuaType.Function)] DynValue func) {
		if (Owner.SetCallback(func)) {
            Log($"Registered on-execute function [{ToUsefulString(func, true)}]", LogTag.CallbackRegistration);
		}
		else {
            ...
            Log($"Got a {descriptor} instead of a function", LogTag.CallbackRegistration);
